...
// frmYM2610.cs's screenDrawParams path calls. YM2610 (OPNB, the Neo Geo's FM+SSG+ADPCM
// chip) has the same 19-channel shape as YM2608 (OPNA): FM 0-5, Ch3-extended-mode slots
// 6-8, SSG 9-11, then a single ADPCM-B playback channel at 12 and a 6-voice ADPCM-A
// ("rhythm") section at 13-18 - functionally almost identical to YM2608's own layout, just
// with ADPCM-A/-B instead of YM2608's built-in sample-based rhythm + single ADPCM. Nearly
// every drawBuff.cs function this chip needs (KeyBoardOPNA/InstOPNA/Volume/VolumeShort/Pan/
// TnOPNA/font4Hex16Bit/font4HexByte/font4Hex12Bit/font4Int2/font4Int3/LfoSw/LfoFrq/Nfrq/
// Efrq/Etype/drawNESSw) is copied verbatim from DrawBuffYm2608.cs rather than referenced
// cross-file, per this port's per-chip self-containment convention. What's genuinely
// different from YM2608: ChYM2610_P's unnumbered-badge offset is 88 (not YM2608's 64),
// ChYM2610Rhythm_P labels its 6 ADPCM-A voices "A1".."6" (not YM2608's B/S/C/H/T/R), and
// VolumeYM2610Rhythm/PanYM2610Rhythm use YM2610's own row/offset constants (row 13, not
// YM2608's row 14).
//
...
{
    public static class DrawBuffYm2610
    {
        ...
        // drawBuff.cs:1290 VolumeYM2610Rhythm - ADPCM-A ("rhythm") section's LED bar, fixed
        // row (8*13), x derived from the ADPCM-A channel index (0-5), c selects mono/L/R.
        public static void VolumeYm2610Rhythm(PixelScreen screen, int rhythmCh, int c, ref int ov, int nv)
        {
            if (ov == nv) return;
            ...
            if (c == 1 || c == 2) { t = 4; }
            if (c == 2) { sy = 4; }
            int x = rhythmCh * 4 * 15 + 6 * 4 + 1;

            for (int i = 0; i <= 19; i++)
            {
                VolumeP(screen, x + i * 2, sy + 8 * 13, 1 + t);
            }

            for (int i = 0; i <= nv; i++)
            {
                VolumeP(screen, x + i * 2, sy + 8 * 13, i > 17 ? 2 + t : 0 + t);
            }
        ...
        // drawBuff.cs:3801 drawFont4Int - leading-zero-blanked numeric readout.
        ...
        // drawFont4Hex16Bit/font4Hex16Bit - FM/SSG/ADPCM channels' packed freq readout.
        ...
        // drawBuff.cs:3520 drawNESSw - generic 4x8 on/off icon (SSG hardware-envelope flag).
        ...
        // drawBuff.cs:1979 PanYM2610Rhythm - ADPCM-A section's per-channel pan icon, fixed
        // row (8*13), x derived from the ADPCM-A channel index.
        public static void PanYm2610Rhythm(PixelScreen screen, int rhythmCh, ref int ot, int nt, ref int otp, int ntp)
        {
            if (ot == nt && otp == ntp) return;
            DrawPanP(screen, rhythmCh * 4 * 15 + 17, 8 * 13, nt);
            ot = nt;
            otp = ntp;
        }

        // drawBuff.cs:4796 ChYM2610_P - channel badge. FM (ch<6): number badge "1".."6".
        // FM-EX op slots (6<=ch<9): number badge "1".."3" at a different sprite-sheet
        // offset. SSG (9<=ch<12): wide icon, no number. ADPCM-B (ch==12): another
        // fixed-offset wide icon at sheet offset 88 (YM2608's equivalent uses offset 64 -
        // the one genuine sprite-offset difference from ChYM2608_P).
        private static void ChYm2610P(PixelScreen screen, int x, int y, int ch, bool mask)
        {
            ...
            {
                screen.DrawIntArray(x, y, src.Pixels, 128, 88, 0, 24, 8);
            }
        }
        ...
        // drawBuff.cs:2401 Ch3YM2610 - dirty-diff wrapper (only ever called with ch==2).
        ...
        // drawBuff.cs:4820 ChYM2610Rhythm_P - single/double-letter ADPCM-A-channel label
        // ("A1", "2".."6"), fixed row, x derived from the channel index.
        private static void ChYm2610RhythmP(PixelScreen screen, int x, int y, int ch, bool mask)
        {
            int t = mask ? 1 : 0;
            switch (ch)
            {
                case 0: DrawFont4(screen, x + 15 * 0 * 4, y, t, "A1"); break;
                case 1: DrawFont4(screen, x + 15 * 1 * 4 + 4, y, t, "2"); break;
                case 2: DrawFont4(screen, x + 15 * 2 * 4 + 4, y, t, "3"); break;
                case 3: DrawFont4(screen, x + 15 * 3 * 4 + 4, y, t, "4"); break;
                case 4: DrawFont4(screen, x + 15 * 4 * 4 + 4, y, t, "5"); break;
                case 5: DrawFont4(screen, x + 15 * 5 * 4 + 4, y, t, "6"); break;
            }
        }

        // drawBuff.cs:2414 ChYM2610Rhythm - dirty-diff wrapper, fixed row (8*13).
        public static void ChYm2610Rhythm(PixelScreen screen, int ch, ref bool? om, bool? nm)
        {
            if (om == nm) return;
            ChYm2610RhythmP(screen, 1, 8 * 13, ch, nm ?? false);
            om = nm;
        }

        // drawBuff.cs:828 InstOPNA - the 11-params-per-operator instrument/operator table
        // (same shape as YM2203/YM2608; see DrawBuffYm2203.cs's Inst comment).
