...
// frmC352.cs's screenDrawParams path calls. C352 (Namco's later 32-channel 4-operator-free
// PCM sample player, quad-output front/rear) is laid out one 8px row per channel across 32
// channels, each showing a keyboard/note readout, FOUR independent volume LED bars
// (front-L/front-R/rear-L/rear-R, via the raw-pixel-position VolumeXY rather than the
// row-index Volume/VolumeToC140 overload), a byte-packed L/R pan icon, 16 on/off flag icons
// (this chip's per-channel control register has twice as many flag bits as C140's), and hex
// readouts of sample frequency/bank/start/end/loop addresses.
//
// Data source: reads chipRegister.pcmRegisterC352[chipId] directly (already a public
// ChipRegister field, mirroring Audio.GetC352Register) and chipRegister.readC352(chipId)
// (already public, forwards to mds.ReadC352Flag - mirroring Audio.GetC352KeyOn) - no new
// getter needed for this chip, like C140.
//
...
{
    public static class DrawBuffC352
    {
        ...
        // multiplied out inside), unlike VolumeToC140's fixed-x row-index overload. Used four
        // times per channel here (front L/R at x=105, rear L/R at x=115).
        ...
        // drawBuff.cs:1638 KeyBoardToC352 - row-index overload, same shape as C140's but with
        // note-name text drawn at x=500/516 instead of C140's x=396/412 (planeC352 is wider).
        public static void KeyBoardToC352(PixelScreen screen, int row, ref int ot, int nt)
        {
            if (ot == nt) return;
        ...
        // overload (fixed x=24). Same original function already ported (independently) in
        // DrawBuffHuc6280.cs/DrawBuffC140.cs - duplicated again here per this port's
        ...
        // drawBuff.cs:4411 ChC352_P - same badge offset/2-digit font as C140's ChC140_P
        // (16,0,16,8 + drawFont4 "d2").
        private static void ChC352P(PixelScreen screen, int x, int y, int ch, bool mask)
        {
        ...
        public static void ChC352(PixelScreen screen, int ch, ref bool? om, bool? nm)
        {
            if (om == nm) return;
            ChC352P(screen, 0, 8 + ch * 8, ch, nm ?? false);
            om = nm;
        }
    }
}
