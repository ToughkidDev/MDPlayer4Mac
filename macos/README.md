...
#### ✅ 활성 칩별 볼륨 믹서 (2026-08-22)

원본 Windows `frmMixer2`의 픽셀 믹서 화면을 Avalonia에 옮겼습니다. 재생을 시작하면
플레이어에 `볼륨 뷰`와 `채널 뷰` 버튼이 활성화되며, 두 화면은 같은 자리를 교체해서
사용합니다. 따라서 볼륨 뷰를 열 때 채널 시각화가 겹치거나 창을 불필요하게 크게
만들지 않습니다.

- `rFader`/`rFont_03` 원본 스프라이트와 −192 ~ +20 dB 변환식을 유지하면서,
  세로 노브를 회전한 긴 가로 페이더로 표시합니다. 마스터와 일반 칩은 Windows 판과
  같은 서로 다른 노브 그래픽을 사용합니다.
- 첫 번째 행은 전체 출력에 적용되는 `MST` 마스터 볼륨이며, 나머지 활성 칩은 한 행씩
  세로로 쌓입니다. 가로 전체를 쓰는 긴 이동 거리에서 드래그 또는 마우스 휠로 세밀하게
  조절할 수 있습니다. 볼륨 뷰는 현재 채널 뷰의 실제 폭을 읽어 같은 창 폭으로
  재구성하고, 페이더 길이도 함께 맞춥니다.
- 볼륨 뷰 아래의 `볼륨 리셋` 버튼은 VGM 파일의 `Volume Modifier`와 v1.70+ 칩별
  볼륨 헤더를 다시 적용합니다. 파일에 해당 값이 없으면 마스터와 활성 칩 모두 0 dB로
  복원합니다.
  듀얼 칩 VGM은 `칩명#1`/`칩명#2` 행을 각각 표시하며, 드래그·휠 조절과 파일 기본값
  리셋도 두 인스턴스에 독립적으로 적용됩니다.
- NES/FDS/DMC는 공유 `nes_intf`의 서브믹서를 직접 조절해 각 음원군을 독립적으로
  조절합니다.
- 조절값은 다음 곡에도 즉시 적용되고, 앱을 닫을 때 기존 `Setting.xml`의 `Balance`
  값(마스터 포함)으로 저장됩니다.

#### ✅ Windows 스타일 전송 버튼 (2026-08-22)

플레이어 상단은 원본 `frmMain`과 같은 16×16 스프라이트(`cc` 정상, `ch` 호버,
`ci` 활성)를 2배 픽셀 배율로 표시합니다. 첫 줄은 정지·일시정지·이전·느리게·재생·
빠르게·다음, 둘째 줄은 파일 열기·볼륨 뷰·채널 뷰·루프입니다. 여러 파일을 한 번에 열면
이전/다음이 그 대기열을 이동합니다. 원본과 마찬가지로 느리게/빠르게는 탐색이 아니라
재생 속도를 0.25×~4× 범위에서 바꾸며, 일시정지는 Core Audio 큐를 멈췄다가 이어서
재생합니다. 루프는 VGM 파일의 루프 지점을 보존해 반복하고, 해당 지점이 없는 파일은
곡 처음부터 다시 재생합니다. `자동 재생` 체크박스를 켜면 파일을 선택한 직후 재생합니다.

음악 파일을 Finder에서 창 또는 재생 목록 영역에 드롭하면 목록에 추가됩니다. 목록은
선택으로 재생 대상을 바꾸고 더블클릭으로 즉시 재생할 수 있으며, 전송 버튼의 이전/다음도
같은 목록을 이동합니다.
...
- 원본과 달리 **칩당 기본 소프트웨어 에뮬레이터 하나만** 배선합니다. 듀얼 칩 VGM은
  두 `Chip.ID`를 보존해 둘 다 연주하지만, 원본처럼 칩별로 여러 에뮬레이터 백엔드 중
  선택하거나 실제 하드웨어로 출력하는 기능은 범위 밖입니다. 따라서
  `CompatShims.cs`의 `RealChip`/`RC86ctlSoundChip`은 계속 no-op입니다.
...
- **VGM EOF 오프셋 호환성 수정 (2026-08-22)**: `Vgm`(`Driver/vgm.cs`)이
  VGM 헤더 0x04의 EOF 오프셋을 절대값으로 해석해, 스펙대로
  `파일길이-4`를 기록한 파일의 마지막 4바이트를 건너뛰던 문제를 수정했습니다.
  이제 필드값에 4를 더해 파일 기준 위치로 바꾸며, 손상된 파일의 과도한 값은
  실제 버퍼 길이로 제한합니다. 따라서 `nes-apu-tone.vgm`처럼 이전 엔진을
  우회하려고 EOF에 전체 파일 길이를 넣었던 픽스처도 계속 재생되면서,
  정상 VGM 파일의 마지막 명령도 누락되지 않습니다.
...
원본의 별난 동작을 그대로 보존)/OKIM6258(OKI MSM6258 ADPCM 보이스
합성 칩 — 지금까지 이식한 PCM 계열 칩 중 처음으로 진짜 단일 채널이라
`Channel[]` 배열 대신 직접 필드를 쓰는 칩)/OKIM6295(OKI MSM6295 ADPCM
샘플 재생 칩, 아케이드 기판에서 널리 쓰임 — 팬 표시가 없는 첫 PCM
계열 칩)/PCM8(X68000의 "PCM8" 8보이스 샘플 드라이버 규약, MXDRV/ZMS/RCS
시퀀스 음악 포맷용 — MpcmX68k와 같은 종류의 생성자 시그니처 예외)/
QSound(캡콤 CQ-SPX67610, 많은 CPS1/CPS2 아케이드 기판에서 쓰임 —
16 PCM + 3 ADPCM = 19채널, 공유 에코/리버브 유닛 상태까지 표시)가
이번 라운드에서 완료됐습니다 — 이로써 NES 계열 8종(NESDMC/FDS/MMC5/
VRC6/VRC7/N106/DMG + 이미 완료된 S5B(FME-7))과 WF 계열 2종(HuC6280/
K051649)이 모두 끝났고, PCM 계열(~15종 중 PWM은 원본 미구현이라
~14종 실질 대상)도 C140/C352/GA20/K053260/K054539/MegaCD/MpcmX68k/
MultiPCM/OKIM6258/OKIM6295/PCM8/QSound/Rf5c68/SegaPCM까지 14종 전부
완료했습니다.
**PWM(32X
내장 PWM 사운드 출력)은 이번 라운드에서 확인해보니 원본 Windows판
`frmPWM.cs`가 `InitializeComponent()`만 호출하는 완전히 빈 스텁이고
(`screenChangeParams`/`screenDrawParams`/배경 이미지/픽셀 박스 전혀
없음), `frmMain.cs`를 포함해 어디에서도 이 폼을 생성하지 않는 죽은
코드라서 — 즉 원본 자체에 포팅할 시각화 로직이 없어서 — 범위에서
제외했습니다.**
...
- **MultiPCM(야마하 YMW258-F, 세가 아케이드/콘솔 기판에서 널리 쓰인
  ...
  모두 원본 그대로 포팅했습니다.
- **OKIM6258(OKI MSM6258 ADPCM 보이스 합성 칩): 이번 라운드에서 신규
  구현, 아직 실기 미검증. 지금까지 이식한 PCM 계열 칩 중 처음으로
  진짜 단일 채널 칩입니다.** 다른 모든 PCM 계열 칩(C140/C352/GA20/
  K053260/K054539/MegaCD/MultiPCM)은 `Channel[]` 배열을 쓰지만, 이
  칩은 실제 하드웨어처럼 모노 ADPCM 음성 출력 하나를 2비트 팬
  레지스터로 L/R에 나누는 구조라서 `MDChipParams.OKIM6258`이 배열 없이
  직접 필드(`pan`/`masterFreq`/`divider`/`pbFreq`/`volumeL`/`volumeR`/
  `mask`)를 씁니다. 8px 행 하나에 원시 단일 타일 팬 아이콘, 5자리
  0-패딩 10진 표시 3개(마스터 클록 주파수 kHz, 클록 분주비, 그로부터
  계산된 재생 주파수 kHz), 독립된 L/R 볼륨 LED 바, 채널 번호 텍스트가
  없는 고정 배지 아이콘(채널이 하나뿐이라 번호가 필요 없음)이
  나열됩니다. **데이터 소스**: `chipRegister.GetOKIM6258Register(chipId)`
  (새 한 줄짜리 포워딩 게터 — `mds.ReadOKIM6258Status`를 감싸기만
  하면 됐는데, 이 포트의 `ChipRegister`가 `mds`를 private으로 유지하기
  때문에 래퍼가 필요했습니다)를 직접 호출합니다. **클록 처리
  불필요**: 원본 `screenChangeParams`는 표시되는 마스터/분주/재생
  주파수와 볼륨 모두 레지스터 상태(`master_clock`/`divider`/
  `data_in`/`status`/`pan`)만으로 계산하고 `Audio.ClockXxx` 프로퍼티는
  전혀 읽지 않습니다.
- **OKIM6295(OKI MSM6295 ADPCM 샘플 재생 칩, 아케이드 기판에서 널리
  쓰임 — NMK112 뱅크 전환 변형 포함): 이번 라운드에서 신규 구현, 아직
  실기 미검증. 지금까지 이식한 PCM 계열 칩 중 처음으로 팬 표시가 없는
  칩입니다.** 채널 4개, 채널당 8px 행 하나씩 채널 배지, 단일 컬럼
  볼륨 LED 바(이 칩의 출력은 팬이 없어서 L/R로 나뉘지 않음), 20비트
  샘플 시작/끝 주소 16진 표시가 나오고, 4개 채널 행 아래에 32비트
  마스터 클록 16진 표시, pin7 상태 16진 표시, 채널당 하나씩 NMK112
  뱅크 선택 16진 표시 4개가 나옵니다. **데이터 소스**:
  `chipRegister.GetOKIM6295Info(chipId)`를 직접 호출합니다 — 사실
  이 메서드는 이전의 부분 포팅 과정에서 이미 `ChipRegister.cs`에
  추가돼 있었지만 `internal`로 선언돼 있어서 별도 어셈블리인
  `MDPlayerUI`에서는 볼 수 없었습니다(`MDPlayerCore.csproj`와
  `MDPlayerUI.csproj`가 분리된 프로젝트라서). 그래서 새 래퍼를
  추가하는 대신 이 메서드의 접근 제한자를 `public`으로 넓혔습니다 —
  지금까지의 "새 게터 추가" 패턴과는 다른 종류의 수정입니다. **클록
  처리 불필요**: 원본 `screenChangeParams`는 클록 값을 전혀 읽지
  않습니다.
- **PCM8(X68000의 "PCM8" 8보이스 샘플 드라이버 규약, MXDRV/ZMS/RCS
  시퀀스 음악 포맷용): 이번 라운드에서 신규 구현, 아직 실기 미검증.
  MpcmX68k와 같은 종류의 아키텍처 예외가 필요했던 두 번째 칩입니다.**
  원본 `frmPCM8.cs`도 `ChipRegister`를 거치지 않고 `Audio.DriverVirtual`을
  `MXDRV`/`ZMS`/`RCS`로 캐스팅해 드라이버 내부의 `pcm8St` 필드를 직접
  읽습니다. 이 포트도 `MpcmX68kVisualizer`처럼 `(baseDriver driver)`
  생성자를 쓰고, `driver is MXDRV`/`driver is ZMS` 패턴 매칭으로
  분기합니다. **MpcmX68k와의 차이점**: 원본은 세 번째 드라이버 포맷인
  RCS도 분기하지만, 이 포트에는 RCS 드라이버 클래스 자체가 전혀
  이식돼 있지 않아서(`macos/MDPlayerCore/Driver/RCS` 디렉터리가 존재하지
  않음) 패턴 매칭할 타입이 없습니다 — 존재하지 않는 타입을 참조하는
  죽은 코드를 억지로 포팅하는 대신 그 분기를 통째로 생략했습니다.
  **enmInstrumentType 예외**: 이 칩도 MpcmX68k처럼 대응하는
  `enmInstrumentType` 항목이 아예 없어서(믹서에 섞이는 MDSound 칩
  인스턴스가 아니라 드라이버 내부 표시 전용이라서), `MainWindow.axaml.cs`
  는 어떤 칩 클록이 아니라 `session.Driver`의 실제 타입(`MXDRV`인지
  `ZMS`인지)으로 표시 여부를 판단합니다. **채널 16개 중 8개만 실제로
  갱신되는 원본의 별난 동작 보존**: `MDChipParams.PCM8`은
  `Channel[16]`을 갖고 `screenDrawParams`는 항상 16행을 모두 그리지만,
  `screenChangeParams`는 `pcm8St.Length`(고정 8)만큼만 순회해서 채널
  8~15는 항상 기본값(유휴) 상태로 남습니다 — 포트가 만든 버그가 아니라
  원본 자체의 구조입니다. 4개의 32비트 16진 표시(샘플 포인터/길이),
  2자리 10진 표시 3개(PCM 모드/재생 레이트/원시 볼륨 레지스터), 원시
  단일 타일 팬 아이콘, 볼륨 LED 바, 채널 배지가 나열됩니다.
- **QSound(캡콤 CQ-SPX67610, 많은 CPS1/CPS2 아케이드 기판에서 쓰임):
  이번 라운드에서 신규 구현, 아직 실기 미검증.** 채널 19개(PCM
  16 + ADPCM 3), 채널당 8px 행 하나씩 에코/주파수/뱅크/샘플 시작·끝/
  루프 시작 16비트 16진 표시(PCM 채널만), 바이트로 압축된 2타일 팬
  아이콘(PCM 채널만 — ADPCM 채널의 `PanType2` 호출은 원본에서 주석
  처리돼 있음), 독립된 L/R 볼륨 LED 바(원시 픽셀 좌표 `VolumeXY`
  오버로드, 19채널 전부), 키보드/노트 표시(PCM 채널만), 채널 배지(19채널
  전부)가 나열되고, 17~18행에는 ADPCM 채널 행의 남는 칼럼을 이용해
  칩 공유 에코/리버브 유닛 상태(피드백/끝 위치/딜레이 업데이트/다음
  상태, wet/dry 각각 L/R 딜레이+볼륨)가 추가로 표시됩니다. **데이터
  소스**: `chipRegister.getQSoundRegister(chipId)`를 직접 호출합니다 —
  이미 `ChipRegister`의 public 메서드라(Windows판 `Audio.
  GetQSoundRegister`도 `chipRegister.getQSoundRegister`를 그대로
  포워딩) 새 게터가 필요 없었습니다. **노트 계산은 공유 헬퍼 재사용**:
  `Common.searchSegaPCMNote`를 직접 호출합니다 — 원본 자체가 이 공유
  헬퍼를 쓰고 있어서(이미 YM2413/YM3526/YM3812/Y8950/YMF262/YMF278B/
  VRC7 등 여러 칩이 같은 방식으로 재사용 중), K051649의 예외와 같은
  이유로 복제하지 않고 그대로 재사용했습니다. **원본의 별난 동작
  보존**: 채널 배지(`ChQSound_P`)는 C140/MpcmX68k/PCM8이 쓰는
  `n*4+64` 특수 오프셋 `DrawFont4Int2` 대신 그냥 `drawFont4`에
  `"D2"` 포맷 문자열을 넘겨서 그리는, 같은 문제에 대한 다른 접근
  방식을 그대로 보존했습니다. 또한 원본 소스에는 `ChQSoundAdpcm_P`라는
  함수가 있지만 실제로는 PCM/ADPCM 두 루프 모두 그냥 `ChQSound`를
  호출해서 이 함수는 어디서도 호출되지 않는 진짜 죽은 코드라 이
  포트에서는 이식하지 않았습니다.
- **Rf5c68(리코 RF5C68, 아케이드 기판용 독립형 8채널 PCM 샘플 재생 칩 —
  세가 메가 CD 내장 RF5C164와 같은 "리코 8채널 PCM" 계열이지만, 실제로는
  `MDSound.scd_pcm`이 아니라 별도의 `MDSound.rf5c68` 에뮬레이션 코어를
  씀): 이번 라운드에서 신규 구현, 아직 실기 미검증.** 채널 8개, 채널당
  8px 행 하나씩 키보드/노트 표시, 독립된 L/R 볼륨 LED 바, 바이트로
  압축된 2타일 팬 아이콘, 채널 배지가 나열됩니다. **배경 이미지와 배지
  함수까지 MegaCD(RF5C164)와 완전히 재사용**: 원본 Windows 소스 자체가
  이 칩의 배경 스프라이트(`planeC`)와 배지 함수(`ChRF5C164`, 별도의
  `ChRF5C68`가 아님)를 MegaCD 쪽과 그대로 공유하고 있어서, 이 포트도
  새 스프라이트를 만들지 않고 이미 내보낸 `planeC`를 그대로 재사용했고
  배지 함수 이름도 원본을 그대로 따라 `ChRf5c164`로 두었습니다. **데이터
  소스**: `chipRegister.GetRf5c68Register(chipId)` — `ChipRegister.cs`에
  새 한 줄 포워딩 게터를 추가했습니다(`mds.ReadRf5c68Register`로 전달,
  `GetRf5c164Register`/`GetK053260Register` 때와 같은 이유로 필요).
  **클록 처리 불필요**: MegaCD/RF5C164의 `searchRf5c164Note`와 마찬가지로
  이 칩의 `searchRf5c68Note`도 `Audio.ClockXxx`로 나누지 않고 원시
  `step` 주파수 레지스터를 `pcmMulTbl` 기반 표와 직접 비교하며, 전역
  최솟값을 정직하게 찾는 탐색을 그대로 보존했습니다(C140/C352/GA20/
  K053260/K054539가 공유하는 "freq > a일 때마다 계속 덮어쓰는" 별난
  동작과는 다릅니다).
- **SegaPCM(세가, 행온/아웃런/스페이스 해리어 등 80년대 중반 세가
  아케이드 기판에서 널리 쓰인 16채널 PCM 샘플 재생 칩 — PCM 계열의
  마지막 남은 칩): 이번 라운드에서 신규 구현, 아직 실기 미검증.**
  채널 16개, 채널당 8px 행 하나씩 키보드/노트 표시, 독립된 L/R 볼륨
  LED 바, 바이트로 압축된 2타일 팬 아이콘, 채널 배지가 나열됩니다.
  **데이터 소스**: `chipRegister.pcmRegisterSEGAPCM[chipId]`(0x200바이트
  평면 레지스터 이미지)와 `chipRegister.pcmKeyOnSEGAPCM[chipId]`(16개
  실시간 키온 플래그 배열)를 직접 읽습니다 — 둘 다 이미 `ChipRegister`의
  public 필드라(Windows판 `Audio.GetSEGAPCMRegister`/`GetSEGAPCMKeyOn`도
  같은 필드를 그대로 반환) 새 게터가 필요 없었습니다. C140의
  `pcmRegisterC140`/`pcmKeyOnC140`과 같은 모양입니다. **노트 계산은
  공유 헬퍼 재사용**: `Common.searchSegaPCMNote`를 직접 호출합니다 —
  원본 자체가 QSound와 함께 이 공유 헬퍼를 쓰고 있어서 복제하지 않고
  그대로 재사용했습니다. **클록 처리 불필요**: `searchSegaPCMNote`는
  미리 나눈 비율("ml" = 원시 델타타임 바이트 / 256.0)을 `pcmMulTbl`
  기반 표와 직접 비교하며, 전역 최솟값을 정직하게 찾는 탐색을
  수행합니다(MegaCD/RF5C164/Rf5c68와 같은 방식이고, C140/C352/GA20/
  K053260/K054539가 공유하는 "freq > a일 때마다 계속 덮어쓰는" 별난
  동작과는 다릅니다). **원본의 별난 동작 보존**: `voldelay`라는
  프레임 카운터(3에서 시작해 매 `ScreenChangeParams` 호출마다 1씩
  줄고, 음수가 되면 5로 리셋)가 키온되지 않은 채널의 볼륨 감쇠
...
5. **MDPlayerUI 기능 확장**: 이제 파일 하나 열기/재생/정지와 활성 칩별 볼륨
   조절을 지원합니다. 재생 목록, 재생 시간 표시/탐색바, 최근 파일 목록 같은 실사용에
