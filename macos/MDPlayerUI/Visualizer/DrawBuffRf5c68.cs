...
// frmRf5c68.cs's screenDrawParams path calls. RF5C68 is the standalone arcade-board PCM chip
// (cousin of the Sega Mega-CD's built-in RF5C164 - same "ricoh 8-channel PCM" family, but a
// genuinely different MDSound.rf5c68 chip class/emulation core, not MDSound.scd_pcm). One 8px
// row per channel across 8 channels, each showing a keyboard/note readout, independent L/R
// volume LED bars, a byte-packed 2-tile pan icon, and a channel badge - reusing the exact same
// background sprite (planeC) and even the same-named ChRF5C164 badge function as
// DrawBuffMegaCD.cs, since the original Windows source itself reuses that function verbatim
// for this chip rather than defining a distinct ChRF5C68.
//
// Data source: reads chipRegister.GetRf5c68Register(chipId) - a new one-line forward added to
// ChipRegister.cs alongside this file, forwarding to mds.ReadRf5c68Register (mirroring
// Audio.GetRf5c68Register - this port's ChipRegister keeps mds private, same reason a wrapper
// was needed for GetRf5c164Register/GetK053260Register before it).
//
...
{
    public static class DrawBuffRf5c68
    {
        ...
        // drawBuff.cs:4477 ChRF5C164_P / :2156 ChRF5C164 - the same badge function
        // DrawBuffMegaCD.cs uses, reused verbatim here because the original Windows source
        // itself calls ChRF5C164 (not a distinct ChRF5C68) for this chip's badge too.
