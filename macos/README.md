...
## 현재 상태 (2026-08-19, 칩 배선 확장: SN76489/YM2612 → 13개 칩)
...
1. **실제 VGM 파일로 검증**: 지금까지는 손으로 만든 최소 테스트 파일만
   썼습니다. vgmrips.net 등에서 실제 게임 VGM(메가드라이브뿐 아니라 이번에
   추가한 아케이드/PC-98/Neo Geo 계열 칩을 쓰는 파일들도)을 받아 돌려보고
   (라이선스/저작권 확인 후) 원본 Windows 빌드와 파형/사운드를 비교해보는 게
   다음 신뢰도 검증 단계입니다. YM2151을 제외한 10개 칩은 아직 실제 오디오로
   검증되지 않았으니 우선순위가 높습니다.
