...
// frmYM3812.cs's screenDrawParams path calls: font4Int2, font4Hex12Bit, KeyBoard, VolumeXY,
// ChYM3812, drawNESSw, plus the shared drawFont8/drawFont4/drawKbn primitives those build
// on. YM3812 (OPL2, the AdLib/Sound Blaster FM chip) has the same channel layout as
// YM3526/OPL1 - 9 FM channels each with their OWN full 2-operator parameter table, plus 5
// fixed-role rhythm channels (BD/SD/TOM/CYM/HH) and 2 ADPCM-related flags (DA/DV) - this
// file is nearly identical to DrawBuffYm3526.cs (same rendering primitives, same badge/
// rhythm-label coordinates); the register-parsing side (Ym3812Visualizer.cs) adds one
// field YM3526 doesn't have, per-operator Waveform Select (WS).
//
...
// masked/unmasked `t` parameter (loading rFont_03 AND rFont_04) because ChYm3812's rhythm
...
{
    public static class DrawBuffYm3812
    {
        ...
        // drawBuff.cs:1171 VolumeXY - x/y are pre-scale-by-4 grid coordinates. YM3812 uses
        ...
        // in frmYM3812.cs passes a literal t=0, so t isn't threaded through here (matches
        ...
        // drawBuff.cs:4574 ChYM3812_P - channel badge (melody: number badge from rType's
        ...
        // separately per-chip).
        private static void ChYm3812P(PixelScreen screen, int x, int y, int ch, bool mask)
        {
            if (ch < 9)
            {
        ...
        // drawBuff.cs:2278 ChYM3812 - dirty-diff wrapper.
        public static void ChYm3812(PixelScreen screen, int ch, ref bool? om, bool? nm)
        {
            if (om == nm) return;
            ChYm3812P(screen, 0, ch < 9 ? 8 + ch * 8 : 8 + 9 * 8, ch, nm ?? false);
            om = nm;
        }
    }
}
